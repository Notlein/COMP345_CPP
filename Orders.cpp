...
bool Advance::validate()
{
	...
	{
		cout << "invalid advance: territory " << tgt_name << " and "
			 << "territory " << source->getName() << " are not adjacent " << endl;
		return false;
	}
...
void Blockade::execute()
{
	if (validate())
	{
		...
		if_executed = true;
		effect = "blockade: number of amies on territory " + target->getName() + " has been doubled, it now has " + to_string(target->getNbArmies()) + " armies, ownership transfer to a Neutral player";
		cout << effect << endl;
	}
}
...
{
	cout <<" size of the orderslist" << this->my_ol->size() << endl;
}
// Destructor
OrdersList::~OrdersList()
{
