...
Territory::Territory(string *name, string *properties) {
	int *a = new int(0);
	Player *o;
...
void Territory::setOwner(Player * new_owner) {
	owner = new_owner;
};
void Territory::setNbArmies(int num) {
	this->nbArmies = new int(num);
};
int Territory::getNbArmies() {
	return *nbArmies;
};
...
vector<string> Territory::getAdjacentTerritories() {
	return *adjTerr;
}
...
Continent::Continent(string *name, int *pts, vector<Territory> *list_terr) {
